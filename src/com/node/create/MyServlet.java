...
public class MyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!request.getParameter("tag1").isEmpty())tags.add(request.getParameter("tag1"));
		if(!request.getParameter("tag2").isEmpty())tags.add(request.getParameter("tag2"));
		if(!request.getParameter("tag3").isEmpty())tags.add(request.getParameter("tag3"));
	...
	int insertvalues(String title,String parentId, String description, String userId) throws ClassNotFoundException, SQLException, ParseException{
		...
		java.sql.PreparedStatement stat= con.prepareStatement("select Levelno,countChildren from Node where NodeID='"+parentId+"'");
		...
		java.sql.PreparedStatement stat1= con.prepareStatement("UPDATE Node SET countChildren="+countChildren+" Where NodeID='"+parentId+"'");
		stat1.executeUpdate();
		String dt=getCuttentDateTime();
		String[] escapetexts=setEcsapeTitleDesc(title,description);
		java.sql.PreparedStatement stat2= con.prepareStatement("INSERT INTO Node(Title,Description,AuthorId,Parent,Levelno,countChildren,UpVote,DownVote,CreationTimeDate) VALUES ('"+escapetexts[0]+"','"+escapetexts[1]+"',"+authorId+","+parentId+","+levelNo+",0,0,0,'"+dt+"');");
		stat2.executeUpdate();
		// get the node id
		java.sql.PreparedStatement stat3= con.prepareStatement("select NodeID from test.Node where title='"+escapetexts[0]+"' and Description='"+escapetexts[1]+"'");
	...
	void inserttagvalues(List<String> tags, int nodeId) throws ClassNotFoundException, SQLException{
		Class.forName("com.mysql.jdbc.Driver");
		Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/test");
		//Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/orbits?" +"user=orbits&password=orbits");
		//get all the tags and id's
		java.sql.PreparedStatement stat= con.prepareStatement("select * from Tags");
		ResultSet result= stat.executeQuery();
		...
		while(it.hasNext()){
			...
			if(result.getString("Tagname").toString().equalsIgnoreCase(inputTag)){
				tagexists=true;
		...
		if(tagexists){
			...
			java.sql.PreparedStatement stat2= con.prepareStatement("INSERT INTO NodeTagsCon VALUES ("+tagid +","+nodeId+")");
			stat2.executeUpdate();
		}else{
			//insert into tag table
			java.sql.PreparedStatement stat2= con.prepareStatement("INSERT INTO Tags(Tagname) VALUES ('"+inputTag.toLowerCase() +"')");
			stat2.executeUpdate();
			//get tag id
			java.sql.PreparedStatement stat4= con.prepareStatement("select TagID from Tags where Tagname='"+inputTag.toLowerCase() +"'");
			ResultSet result4= stat4.executeQuery();
			result4.first();
			tagid=result4.getInt(1);
			//insert into node connection table
			java.sql.PreparedStatement stat3= con.prepareStatement("INSERT INTO NodeTagsCon VALUES ("+tagid+","+nodeId+")");
			stat3.executeUpdate();
	...
	String getCuttentDateTime() throws ParseException{
